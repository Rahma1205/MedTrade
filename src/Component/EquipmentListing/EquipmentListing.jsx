    ...
    <section className="py-5" dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="container">
        <div className="d-flex justify-content-between align-items-center mb-4">
          ...
          {/* Uncomment if userData is available */}
          {/* {userData && (
            <Link to="/AddEquipment" className="btn btn-outline-primary px-4 py-2 fw-semibold">
              {t('addEquipment')}
            </Link>
          )} */}
        </div>
...
{t(`condition.${conditionMap[item.condition] || item.condition}`)}
        ...
        <div className="text-center mt-4">
          <Link to="/EquipmentListingAll" className="btn btn-outline-primary px-4 py-2 fw-semibold">
            {t('viewAllEquipment')}
          </Link>
        </div>
      </div>
